...
// Initialize products
// Only include products for women (category or categories includes 'women')
products = sampleProducts.filter(p => (p.category === 'women' || (Array.isArray(p.categories) && p.categories.includes('women'))));
...
function showToast(message, type = 'info') {
    ...
    if (!toastContainer || toastContainer.offsetParent === null) {
        // Fallback: show alert if toast container is missing or hidden
        alert(message);
        return;
    }
    ...
    bestSellers.forEach(product => {
        ...
        productCard.innerHTML = `
            <div class="product-image" style="position:relative;">
                ...
                <button class="btn btn-outline quick-view-btn best-selling-quick-view" data-product-id="${product.id}" aria-label="${currentLanguage === 'ar' ? 'عرض سريع لـ' : 'Quick view for'} ${productName}" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);opacity:0;pointer-events:none;transition:opacity 0.2s;z-index:2;">${currentLanguage === 'ar' ? 'عرض سريع' : 'Quick View'}</button>
            </div>
            <div class="product-info">
                <h3 class="product-name">${productName}</h3>
                ...
                <div class="stock-number">
                    ${currentLanguage === 'ar' ? 'الكمية المتوفرة: ' : 'Stock: '}<span>${product.stock}</span>
                </div>
                <div class="product-actions">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    currentLanguage = localStorage.getItem('alqady_language') || 'en'; // Ensure language is loaded correctly
...
window.addEventListener('beforeunload', function() {
    // localStorage.removeItem('alqady_language'); // Comment out this line if you want language to persist
});
